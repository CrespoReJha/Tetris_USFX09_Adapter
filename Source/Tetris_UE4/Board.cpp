...
void ABoard::NewPieces()
{
    ...
    if (CurrentPieces->Index >= 7) {
        int num = FMath::RandRange(0, 10);
        //int num = 2;
        if (num < 3) {
            GEngine->AddOnScreenDebugMessage(-1, 3, FColor::Green, TEXT("Se genero una PIEZA LOCA"));
            Adaptador = NewObject<UAdaptadorMovimientoAleatorio_CA>(CurrentPieces);
            Adaptador->SetLimitesMovimiento(0.0f, 10.0f, 10.0f);
            Adaptador->RegisterComponent();
        }
    }
